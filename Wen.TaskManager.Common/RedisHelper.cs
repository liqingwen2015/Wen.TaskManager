...
using Wen.TaskManager.Common.Enums;

#endregion

namespace Wen.TaskManager.Common
{
    /// <summary>
    /// Redis 助手
    /// </summary>
    public class RedisHelper
    {
        ...
        /// <summary>
        /// 根据偏移量来设置字符串的 bit 值（当键不存在时，将创建一个新的字符串值）
        /// </summary>
        /// <param name="redisKey"></param>
        /// <param name="offset"></param>
        /// <param name="bit"></param>
        /// <returns></returns>
        public bool StringSetBit(string redisKey, long offset, bool bit)
        {
            redisKey = AddKeyPrefix(redisKey);
            return _db.StringSetBit(redisKey, offset, bit);
        }

        public bool StringGetBit(string redisKey, long offset)
        {
            redisKey = AddKeyPrefix(redisKey);
            return _db.StringGetBit(redisKey, offset);
        }

        public long StringBitCount(string redisKey, long start = 0L, long end = -1L)
        {
            redisKey = AddKeyPrefix(redisKey);
            return _db.StringBitCount(redisKey, start, end);
        }
        ...
        /// <param name="bit"></param>
        /// <returns></returns>
        public Task<bool> StringSetBitAsync(string redisKey, long offset, bool bit)
        {
            redisKey = AddKeyPrefix(redisKey);
            return _db.StringSetBitAsync(redisKey, offset, bit);
        }
